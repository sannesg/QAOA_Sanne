...
class Planner:
    ...
    def __init__(self, model="gpt-4", temperature=0):
        ...
        self.tools = [
            Tool(
                name="WhichAgent",
                func=self.choose_agent,
                description="Choose which agent to use for the task.",
            )
        ]
...
Remember: Be concise, focused, and precise.""",
        ...
        filled_prompt = full_prompt.format(
            description=description, context=self.context
        )
        return self.agent.run(filled_prompt)
...
while True:
    ...
    if "case 1" or "case 0" in result.lower():
        ...
        response = result
    """
    response = result  # For now, we just return the result directly
    print("\nAnswer:")
    print(response)
    print("-" * 40)
